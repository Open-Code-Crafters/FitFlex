...
const Blog = () => {
  ...
  // Initialize state for likes and comments for each post
  const [likes, setLikes] = useState(blogPosts.map(() => 0));
  const [showCommentBox, setShowCommentBox] = useState(blogPosts.map(() => false)); // To toggle comment box visibility
  const [comments, setComments] = useState(blogPosts.map(() => "")); // To store the user's comments
